...
const sendMonthlyOffers = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    if (users.length === 0 || deals.length === 0) {
      return {
        skipped: true,
        reason: "No users or deals found",
      };
    }
...
const autoAssignRider = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      const available = await prisma.deliveryPartner.findFirst({
        where: {
          isActive: true,
          id: {
            notIn: busyRiderIds as string[],
          },
        },
      });
      ...
      history.push({
        status: "Assigned",
        note: `Auto-assigned to ${available.name}`,
        timestamp: new Date(),
      });
      ...
      await prisma.order.update({
        ...
        data: {
          deliveryPartnerId: available.id,
          deliveryOtp: otp,
          status: "Assigned",
